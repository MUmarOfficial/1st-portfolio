...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const addItemAnimations = useCallback(
    (tl: gsap.core.Timeline, itemEls: Element[], numberEls: Element[], panelInsertTime: number, panelDuration: number) => {
  ...
  const addSocialAnimations = useCallback(
    (tl: gsap.core.Timeline, socialTitle: Element | null, socialLinks: Element[], panelInsertTime: number, panelDuration: number) => {
      if (!socialTitle && !socialLinks.length) return;
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    addItemAnimations(tl, itemEls, numberEls, panelInsertTime, panelDuration);
    addSocialAnimations(tl, socialTitle, socialLinks, panelInsertTime, panelDuration);

    openTlRef.current = tl;
    return tl;
  }, [addItemAnimations, addSocialAnimations]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <header
          className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-2 bg-transparent pointer-events-none z-20"
          ...
          >
            ...
            >
              <span ref={textInnerRef} className="sm-toggle-textInner flex flex-col leading-none">
                {textLines.map((l) => (
                  <span className="sm-toggle-line block h-[1em] leading-none" key={`line-${l}`}>
                    {l}
                  </span>
                ))}
              </span>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-black/40 flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-md border-l border-white/10 pointer-events-auto"
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className="sm-socials-link text-[1.2rem] font-medium text-white/80 no-underline relative inline-block py-0.5 transition-[color,opacity] duration-300 ease-linear"
                      >
                        {s.label}
                      </a>
                    </li>
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; background: rgba(0,0,0,0.4); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-left: 1px solid rgba(255,255,255,0.1); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
